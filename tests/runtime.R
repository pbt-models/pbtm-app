...
speedTable <- function(df, groups, basis = 50) {
  df |> addFracDiff(groups) |> interpolateGermSpeed(groups, basis) |>
    dplyr::mutate(GR = round(1 / Time, 6))
}

dataFor <- list(
  ThermalTime = sampleGermData, Hydrotime = sampleGermData,
  HydrothermalTime = sampleGermData, Aging = sampleAgingData,
  Promoter = samplePromoterData, Inhibitor = sampleInhibitorData
)
...
for (nm in names(dataFor)) {
  ...
  check(paste(nm, "plot builds (no fit)"), buildsClean(buildCdfPlot(spec, df, NULL, 1, identity)))
}

# promoter/inhibitor with log transform
for (nm in c("Promoter", "Inhibitor")) {
  spec <- modelSpecs[[nm]]; df <- dataFor[[nm]]
...
cat(sprintf("\n%s\n", strrep("-", 40)))
if (ok) cat("RUNTIME PLOT CHECKS PASSED ✓\n") else { cat("RUNTIME PLOT CHECKS FAILED\n"); quit(status = 1) }
