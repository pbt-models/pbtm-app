    ...
    function(input, output, session) {
      ...
      output$content <- renderUI({
        ...
        validate(need(
          ready(),
          paste(
            "Please load required data for the inhibitor model analysis. Minimum required columns are:",
            paste(req_cols, collapse = ", ")
          )
        ))

        germInhibitorDosageChoices <- getColChoices(
          data(),
          "GermInhibitorDosage"
        )
        otherTrtCols <- setdiff(
          names(data()),
          c("GermInhibitorDosage", "CumTime", "CumFraction")
        )
