...
ThermalTimeUI <- function() {
  ...
  tagList(
    ...
    div(
      class = "tab-info",
      "The thermal time model assumes a data set with germination temperature as a treatment condition. If you have additional treatments in your dataset, the model will display those treatments and include them in the model calculation which can result on unreliable or unexpected results. Please inform and remove additonal treatments when pertinent in the lower left corner of this section. Note: the model may fail to converge under conditions, try adjusting data or model constraints."
    ),
    uiOutput(ns("content"))
  )
}
    ...
    function(input, output, session) {
      ...
      modelResults <- reactive({
        ...
        suppressWarnings(
          tryCatch(
            {
              model <- nls(
                formula = CumFraction ~ maxCumFrac *
                  pnorm(
      ...
      output$content <- renderUI({
        ...
        validate(need(
          ready(),
          paste(
            "Please load a dataset with required columns for thermal time analysis. Minimum required columns are:",
        ...
        otherTrtCols <- setdiff(
          names(data()),
          c("GermTemp", "CumTime", "CumFraction")
        )
      ...
      output$modelResults <- modelResultsUI(
        ns,
        reactive(rv$lastGoodModel),
        reactive(rv$heldParams)
      )
      ...
      output$plot <- renderPlot(
        {
          ...
          if (is.list(model)) {
            ...
            plt <- plt +
              labs(
                title = "Cumulative germination and thermal time sub-optimal model fit"
              ) +
              mapply(
                function(temp) {
                  stat_function(
