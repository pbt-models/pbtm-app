...
HydrothermalPrimingUI <- function() {
  ...
  tagList(
    ...
    div(
      class = "tab-info",
      "This hydrothermal priming model assumes a germination data set with priming temperature, priming water potential and priming duration as treatment conditions. If you have additional treatments in your dataset, the model will average across those treatments and you may get unreliable or unexpected model results. Note: the model may fail to converge under conditions, try adjusting data or model constraints."
    ),
    uiOutput(ns("content"))
  )
}
    ...
    function(input, output, session) {
      ...
      workingData <- reactive({
        ...
        df <- data() %>%
          ...
          filter(
            PrimingTemp %in% input$primingTempSelect,
          ...
          tryCatch(
            {
              ...
              fθHTP <- function(PM50, TMIN) {
                (PrimingWP - PM50) * (PrimingTemp - TMIN) * PrimingDuration
              }
      ...
      output$content <- renderUI({
        ...
        validate(need(
          ready(),
          paste(
            "Please load required data for hydrothermal priming analysis. Minimum required columns are:",
      ...
      output$plot <- renderPlot(
        {
          ...
          if (is.list(model)) {
            ...
            plt <- plt +
              labs(
                title = "Germination rates and hydrothermal priming model fit"
              ) +
