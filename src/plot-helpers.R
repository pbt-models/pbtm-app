...
buildCdfCurveData <- function(spec, df, params, maxFrac, transform = identity, n = 200) {
  ...
  purrr::pmap_dfr(combos, function(...) {
    ...
    for (f in spec$factors) nd[[f]] <- row[[f]]
    nd$pred <- spec$predict(nd, params, maxFrac = maxFrac, transform = transform)
    nd
  })
}
...
buildCdfPlot <- function(spec, df, model, maxFrac, transform = identity, interactive = FALSE) {
  cfg <- spec$plot
  colorVar <- cfg$colorVar

  plt <- ggplot(df, aes(x = CumTime, y = CumFraction, color = as.factor(.data[[colorVar]]))) +
    addFracToPlot(maxFrac)

  if (!is.null(cfg$shapeVar)) {
    plt <- plt + geom_point(aes(shape = as.factor(.data[[cfg$shapeVar]])), size = 2)
  } else {
  ...
  plt <- plt +
    ...
    scale_x_continuous(breaks = scales::breaks_pretty(6), expand = expansion(c(0, .05))) +
  ...
  if (is.list(model)) {
    curve <- buildCdfCurveData(spec, df, model, maxFrac, transform)
    if (!is.null(cfg$lineTypeVar)) {
      ...
      plt <- plt + geom_line(
        data = curve,
        aes(x = CumTime, y = pred, color = as.factor(.data[[colorVar]]), group = as.factor(.data[[colorVar]]))
      )
    }
    plt <- plt + labs(title = cfg$fitTitle)
    if (!interactive) plt <- addParamsToPlot(plt, spec$annotate(model), 1)
  }
...
buildRatePlot <- function(spec, df, model, interactive = FALSE) {
  cfg <- spec$plot
  validate(need(is.list(model), "Model results not yet available; adjust settings."))
  ...
  plt <- ggplot(df, aes(x = .theta, y = .gr, color = as.factor(.data[[cfg$colorVar]])))

  ptAes <- aes()
  if (!is.null(cfg$shapeVar)) ptAes <- modifyList(ptAes, aes(shape = as.factor(.data[[cfg$shapeVar]])))
  if (!is.null(cfg$sizeVar)) ptAes <- modifyList(ptAes, aes(size = as.factor(.data[[cfg$sizeVar]])))
  ...
  plt <- plt +
    ...
    labs(
      caption = "Generated with the PBTM app",
      x = cfg$xlab, y = "Germination rate",
      color = cfg$colorLab, shape = cfg$shapeLab, size = cfg$sizeLab
    ) +
    guides(color = guide_legend(reverse = cfg$legendReverse, order = 1), linetype = "none") +
    theme_classic() +
    theme(plot.title = element_text(face = "bold", size = 14))
  ...
  if (!interactive) plt <- addParamsToPlot(plt, spec$annotate(model), ymax)
