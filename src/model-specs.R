...
modelSpec <- function(id, label, family, factors, factorLabels, params, predict,
                      annotate, plot, response = if (family == "rate") "GR" else "CumFraction",
                      groups = NULL, transformCol = NULL, subpop = FALSE,
                      doc = NULL, tabInfo = NULL) {
  stopifnot(family %in% c("cdf", "rate"), is.function(predict), is.function(annotate))
  list(
    id = id,
    label = label,
    family = family,
...
modelSpecs <- list(
  ...
  ThermalTime = modelSpec(
    id = "thermalTime", label = "Thermal time", family = "cdf",
    factors = "GermTemp",
    factorLabels = c(GermTemp = "Included temperature levels:"),
    subpop = TRUE, doc = "md/thermal_time.md",
    ...
    plot = list(
      colorVar = "GermTemp", colorLab = "Temperature",
      legendReverse = TRUE, fitTitle = "Cumulative germination and thermal time sub-optimal model fit"
    )
  ),

  Hydrotime = modelSpec(
    id = "hydrotime", label = "Hydrotime", family = "cdf",
    factors = "GermWP",
    factorLabels = c(GermWP = "Included water potential levels:"),
    subpop = TRUE, doc = "md/hydrotime.md",
    ...
    plot = list(
      colorVar = "GermWP", colorLab = "Water potential",
      legendReverse = TRUE, fitTitle = "Cumulative germination and hydrotime model fit"
    )
  ),

  HydrothermalTime = modelSpec(
    id = "hydrothermalTime", label = "Hydrothermal time", family = "cdf",
    factors = c("GermWP", "GermTemp"),
    factorLabels = c(
      GermWP = "Included water potential levels:",
      GermTemp = "Included temperature levels:"
    ),
    subpop = TRUE, doc = "md/hydrothermal-time.md",
    ...
    predict = function(data, p, maxFrac = 1, transform = identity) {
      maxFrac * pnorm(
        q = data$GermWP - (p$ThetaHT / ((data$GermTemp - p$Tb) * data$CumTime)),
        mean = p$PsiB50,
        sd = p$Sigma
      )
    },
    ...
    plot = list(
      colorVar = "GermWP", colorLab = "Water potential",
      shapeVar = "GermTemp", shapeLab = "Temperature",
      lineTypeVar = "GermTemp",
      legendReverse = TRUE, fitTitle = "Cumulative germination and hydrothermal time model fit"
    )
  ),

  Aging = modelSpec(
    id = "aging", label = "Aging", family = "cdf",
    factors = "AgingTime",
    factorLabels = c(AgingTime = "Included aging times:"),
    subpop = TRUE, doc = "md/aging-time.md",
    ...
    plot = list(
      colorVar = "AgingTime", colorLab = "Aging time",
      legendReverse = FALSE, fitTitle = "Cumulative germination and aging model fit"
    )
  ),

  Promoter = modelSpec(
    id = "promoter", label = "Promoter", family = "cdf",
    factors = "GermPromoterDosage",
    factorLabels = c(GermPromoterDosage = "Included promoter dosages:"),
    transformCol = "GermPromoterDosage", subpop = FALSE, doc = "md/promoters.md",
    ...
    annotate = function(res) list(
      ...
      sprintf("~~P[b](50)==%.2f", 10^res$Pb50),
      sprintf("~~sigma==%.3f", res$Sigma),
      sprintf("~~R^2==%.2f", res$PseudoR2)
    ),
    plot = list(
      colorVar = "GermPromoterDosage", colorLab = "Promoter dosage",
      legendReverse = FALSE, fitTitle = "Cumulative germination and promoter model fit"
    )
  ),

  Inhibitor = modelSpec(
    id = "inhibitor", label = "Inhibitor", family = "cdf",
    factors = "GermInhibitorDosage",
    factorLabels = c(GermInhibitorDosage = "Included inhibitor dosages:"),
    transformCol = "GermInhibitorDosage", subpop = FALSE, doc = "md/inhibitors.md",
    ...
    annotate = function(res) list(
      ...
      sprintf("~~I[b](50)==%.2f", 10^res$Ib50),
      sprintf("~~sigma==%.3f", res$Sigma),
      sprintf("~~R^2==%.2f", res$PseudoR2)
    ),
    plot = list(
      colorVar = "GermInhibitorDosage", colorLab = "Inhibitor dosage",
      legendReverse = FALSE, fitTitle = "Cumulative germination and inhibitor model fit"
    )
  ),
  ...
  Hydropriming = modelSpec(
    id = "hydropriming", label = "Hydropriming", family = "rate",
    ...
    subpop = FALSE, doc = "md/hydropriming-time.md",
    ...
    plot = list(
      theta = function(data, res) (data$PrimingWP - res$PsiMin) * data$PrimingDuration,
      xlab = "Hydropriming time",
      colorVar = "PrimingWP", colorLab = "Water potential",
      shapeVar = "PrimingDuration", shapeLab = "Duration",
      fixedSize = 4, legendReverse = TRUE,
      fitTitle = "Germination rates and hydropriming model fit"
    )
  ),

  HydrothermalPriming = modelSpec(
    id = "hydrothermalPriming", label = "Hydrothermal priming", family = "rate",
    ...
    subpop = FALSE, doc = "md/hydrothermal-priming-time.md",
    ...
    plot = list(
      theta = function(data, res) (data$PrimingWP - res$PsiMin) *
        (data$PrimingTemp - res$Tmin) * data$PrimingDuration,
      xlab = "Hydrothermal priming time",
      colorVar = "PrimingWP", colorLab = "Water potential",
      shapeVar = "PrimingTemp", shapeLab = "Temperature",
      sizeVar = "PrimingDuration", sizeLab = "Duration",
      legendReverse = TRUE,
...
for (.nm in names(modelSpecs)) modelSpecs[[.nm]]$modelCol <- .nm
rm(.nm)
