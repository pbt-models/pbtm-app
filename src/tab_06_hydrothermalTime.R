    ...
    function(input, output, session) {
      ...
      output$content <- renderUI({
        ...
        validate(need(
          ready(),
          paste(
            "Please load required data for hydrothermal time analysis. Minimum required columns are:",
        ...
        otherTrtCols <- setdiff(
          names(data()),
          c("GermWP", "GermTemp", "CumTime", "CumFraction")
        )
      ...
      output$plot <- renderPlot(
        {
          ...
          if (is.list(model)) {
            ...
            plt <- plt +
              labs(
                title = "Cumulative germination and hydrothermal time model fit"
              ) +
              mapply(
                function(wp, temp) {
                  stat_function(
