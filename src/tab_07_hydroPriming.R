...
HydroprimingUI <- function() {
  ...
  tagList(
    ...
    div(
      class = "tab-info",
      "This hydropriming model assumes a germination data set with priming water potential and priming duration as treatment conditions. If you have additional treatments in your dataset, the model will average across those treatments and you may get unreliable or unexpected model results. Note: the model may fail to converge under conditions, try adjusting data or model constraints."
    ),
    uiOutput(ns("content"))
  )
}
    ...
    function(input, output, session) {
          ...
          tryCatch(
            {
              ...
              fθHTP <- function(PM50) {
                (PrimingWP - PM50) * PrimingDuration
              }
      ...
      output$content <- renderUI({
        ...
        validate(need(
          ready(),
          paste(
            "Please load required data for hydropriming analysis. Minimum required columns are:",
        ...
        otherTrtCols <- setdiff(
          names(data()),
          c("PrimingWP", "PrimingDuration", "CumTime", "CumFraction")
        )
      ...
      output$plot <- renderPlot(
        {
          ...
          plt <- df %>%
            ...
            geom_point(size = 4, aes(shape = as.factor(PrimingDuration))) +
            scale_y_continuous(
