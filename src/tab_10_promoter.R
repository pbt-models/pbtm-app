...
PromoterUI <- function() {
  ...
  tagList(
    ...
    div(
      class = "tab-info",
      "The promoter model assumes a data set with a promoter at different dosages (increasing values will speed up germination) as a treatment condition. If you have additional treatments in your dataset, please filter those out as you may get unreliable or unexpected model results. Note: the model may fail to converge under conditions, try adjusting data or model constraints."
    ),
    uiOutput(ns("content"))
  )
}
    ...
    function(input, output, session) {
      ...
      output$content <- renderUI({
        ...
        validate(need(
          ready(),
          paste(
            "Please load required data for the promoter model analysis. Minimum required columns are:",
        ...
        otherTrtCols <- setdiff(
          names(data()),
          c("GermPromoterDosage", "CumTime", "CumFraction")
        )
